...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, feature_dim=128):
        ...
        self.projection1 = nn.Linear(512*block.expansion,512*block.expansion)
        self.projection2 = nn.Linear(512*block.expansion,feature_dim)
    ...
    def forward(self, x, mode = 'linear'):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        ...
        if mode in ['projection']:
            return F.normalize(self.forward_projection(out),dim=1)
        elif mode in ['linear']:
            return self.forward_linear(out)
        
    def forward_linear(self,out):
        out = self.linear(out)
        return out

    def forward_projection(self,out):
